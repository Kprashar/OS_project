// in this there is constrain that arival time of first process is should be zero.
#include<stdlib.h>
#include<fcntl.h>
#include<sys/stat.h>
#include<sys/types.h>
...
printf("\n\n\n");
long int k2 = 1;//doing sorting acording to priority
  double b_time2 = 0;
  for(int j=0;j<no_of_process;j++)
  {
  ...
  {
    sum3 += burst_time[i-1];
    waiting_time[i] = sum3 - arival_time[i];
  ...
  {
    sum4 += burst_time[i];
    turnaround_time[i] = sum4 - arival_time[i];
